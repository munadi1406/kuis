...
import { A as AstroError, R as ResponseSentError } from './astro/assets-service_CR-gM4WY.mjs';
...
// const { cookies, redirect } = Astro;
// `context` and `next` are automatically typed
const validate = defineMiddleware(async (context, next) => {
  const { request } = context;
  const url = new URL(request.url);
...
const onRequest$1 = sequence(auth,validate);
