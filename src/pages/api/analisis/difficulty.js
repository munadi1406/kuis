...
export const GET = async ({ params, url }) => {
  try {
    ...
    const questionDifficulty = Object.keys(questionAttemptCounts).map(questionId => {
      ...
      let difficulty = 0;
      if (attemptCount > 0) {
        const p = correctCount / attemptCount; // Proportion of correct answers
    ...
    const averageDifficulty = validDifficulties.reduce((acc, q) => acc + parseFloat(q.difficulty), 0) / validDifficulties.length;
